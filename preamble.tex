...
\newcommand{\CoordsHere}{%
  ...
  \setlength{\Ycenter}{\dimexpr \paperheight-\pagetotal\relax} % 메모, 그림 박스가 삽입되는 중심점
...
\newcommand{\ClampFromCoords}{%
  ...
  \else\ifdim \Yboxtop>\Ytop % 위에 지정된 데드라인보다 박스가 올라가면
    \setlength{\yshiftEdge}{\dimexpr \Ytop-\Yboxtop\relax} % 넘어가는 값(-)을 shift 값으로 가짐 - 아래로 내림
  \fi\fi}

% (옵션) 좌표 디버그시 \PrintCoords로 사용
