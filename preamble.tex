...
\newlength\TopOffset     \setlength{\TopOffset}{110pt} % 위에서 내려올 거리
\newlength\BottomOffset  \setlength{\BottomOffset}{110pt} % 아래에서 올라올 거리

% (1) 임의 콘텐츠의 '절반 높이'를 halfbox에 설정 (측정 전용)
\newcommand{\MeasureHalfFrom}[1]{%
  \sbox{\edgebox}{#1}%
  \setlength{\halfbox}{.5\dimexpr \ht\edgebox+\dp\edgebox\relax}%
}

% (2) 현재 위치 좌표 계산(페이지 좌표계: 위=\paperheight, 아래=0)
\newcommand{\CoordsHere}{%
  \setlength{\Ytop}{\dimexpr \paperheight-\TopOffset\relax}%
  \setlength{\Ybottom}{\BottomOffset}%
  \setlength{\Ycenter}{\dimexpr \paperheight-\pagetotal\relax}%
  \setlength{\Yboxtop}{\dimexpr \Ycenter+\halfbox\relax}%
  \setlength{\Yboxbottom}{\dimexpr \Ycenter-\halfbox\relax}%
}

% (3) 넘침만 보정(아래 넘치면 ↑, 위 넘치면 ↓) → yshiftEdge
\newcommand{\ClampFromCoords}{%
  \setlength{\yshiftEdge}{0pt}%
  \ifdim \Yboxbottom<\Ybottom
    \setlength{\yshiftEdge}{\dimexpr \Ybottom-\Yboxbottom\relax}%
  \else\ifdim \Yboxtop>\Ytop
    \setlength{\yshiftEdge}{\dimexpr \Ytop-\Yboxtop\relax}%
  \fi\fi
}
